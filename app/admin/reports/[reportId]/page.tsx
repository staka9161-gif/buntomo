...
export default async function AdminReportDetailPage({ params }: AdminReportDetailPageProps) {
  ...
  const targetUserAdminHref = report.targetUserId ? `/admin/users/${report.targetUserId}` : null;
          ...
          <section className="rounded-lg border bg-white p-5 shadow-sm">
                ...
                <div className="space-y-1">
                  ...
                  {targetUserAdminHref ? (
                    <span className="block">
                      <Link
                        href={targetUserAdminHref}
                        className="text-xs font-semibold text-gray-700 hover:text-amber-700 hover:underline"
                      >
                        対象ユーザーの管理画面を開く
                      </Link>
                    </span>
            ...
            {targetUserAdminHref ? (
              <div className="mt-4 rounded-md border border-amber-100 bg-amber-50 px-3 py-2 text-sm text-amber-900">
                利用制限などの確認・操作は
                <Link href={targetUserAdminHref} className="mx-1 font-semibold underline">
                  対象ユーザーの管理画面
                </Link>
                で行ってください。
              </div>
